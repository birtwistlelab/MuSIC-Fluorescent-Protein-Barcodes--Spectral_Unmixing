# ====================================== XL Clemson ======================================
# Now we will unmix the testing pool and create its 18x18 plots.
# ====================================== XL Clemson ======================================
...
saving_paths1 = [os.path.join(result_path, '7.UnmixingTransfectedTestingCells', f'test{i + 1}') for i in range(3)]
for path in saving_paths1:
...
# Transpose the RF matrix to align with cell fluorescence values during NNLS
...
for i in range(3):
    ...
    transfected_cell_dict = np.load(os.path.join(loading_path, 'testing_dict.npy'), allow_pickle=True).item()
    ...
    for key, value in sorted(transfected_cell_dict.items()):
        ...
        for each_cell in value:
            ...
            scale_x.append(x[1:]) # Skip first column which is the background (untransfected)
        scale_x = np.array(scale_x)

        total_cells = len(scale_x)
        total_cell_number_dict[key] = total_cells
        ...
        for col in range(scale_x.shape[1]):
            ...
            file_path = os.path.join(saving_paths1[i], '18x18_plots', f'{key[:2]}.pR_{key[3:]}_TestingCells')
            os.makedirs(file_path, exist_ok=True)
            filename = key + '_RF' + str(col) + '.png'
            plt.savefig(os.path.join(file_path, filename))
            plt.close()

    np.save(os.path.join(saving_paths1[i], '7.transfected_Testing_pR_fp_scale_x.npy'), transfected_pR_fp_scale_x,
            allow_pickle=True)
    np.save(os.path.join(saving_paths1[i], '7.transfected_Testing_cell_number.npy'), total_cell_number_dict,
            allow_pickle=True)
