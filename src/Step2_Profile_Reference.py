...
# Fluorescence (FI) of Cells from the 'Unstained' group represent autofluorescence (auto_FI):
...
# For each fluorescent protein, its pure_FI should be calculated by subtracting the auto_FI of the positive cells from
# their total_FI.
fp_MFI_RF = {}
...
# Assign the names of the references:
RF_name = []
for i in range(19):
    if i < 10:
        name = 'RF_fp0' + str(i)
        ...
        name = 'RF_fp' + str(i)
    RF_name.append(name)
# RF_name is ['RF_fp00', 'RF_fp01', 'RF_fp02', 'RF_fp03', 'RF_fp04', 'RF_fp05', 'RF_fp06', 'RF_fp07',
# 'RF_fp08', 'RF_fp09', 'RF_fp10', 'RF_fp11', 'RF_fp12', 'RF_fp13', 'RF_fp14', 'RF_fp15', 'RF_fp16', 'RF_fp17',
# 'RF_fp18']

# Assign the calculated pure MFI to its corresponding reference.
RF = []
...
# Visualize the spectrum of each Reference. From this, the index of the peak channel can be determined.
RF_peak = {}
for i in range(1, 19):
    label = RF_name[i]
    each_ref = RF[i]
    ...
    file_path = os.path.join(result_path, '2.RF_spectrum/')
...
np.save(os.path.join(result_path, '2.RF_peak.npy'), RF_peak, allow_pickle=True)  # dictionary: 'RF_fp01': array([2])
np.save(os.path.join(result_path, '2.fp_MFI_RF.npy'), fp_MFI_RF, allow_pickle=True)  # dictionary, pure_MFI of each reference
np.save(os.path.join(result_path,'2.MFI_RF.npy'), RF)  # list, pure_MFI of each reference
np.save(os.path.join(result_path, '2.RF_name.npy'), RF_name)  # list: 'RF_fp00'
