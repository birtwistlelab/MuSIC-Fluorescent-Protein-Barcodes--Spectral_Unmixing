# ================================================== XL Clemson ==================================================
# To extract the data from the csv files of positive pR-fps by manually gating to create the reference for further
# unmixing
# ================================================== XL Clemson ==================================================
import numpy as np
import pandas as pd
...
# Because the channel order of files extracted from FlowJo differs from Cytek's channel order, the results will be
# rearranged according to Cytek's channel order.

files = glob.glob(os.path.join(path, '*.csv'))

# We set Cytek's channel order as the new_order
new_order = ['V1-A', 'V2-A', 'V3-A', 'V4-A', 'V5-A', 'V6-A', 'V7-A', 'V8-A', 'V9-A', 'V10-A', 'V11-A', 'V12-A', 'V13-A',
...
# We transfected pReceiver (pR) as the negative control, which Cytek refers to as "unstained."
print(len(fp_reference['00.unstained']))

np.save(os.path.join(result_path, '1.fp_reference_2023.npy'), fp_reference, allow_pickle=True)
np.save(os.path.join(result_path,'1.x_axis_channel.npy'), new_order)
